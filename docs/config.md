...
- smtputf8
    ...
    RFC 6531 does not define one. Only enable this if the queue plugin in use
    and the MTA behind it can route non-ASCII addresses; `queue/qmail-queue`
    passes the octets through unchanged, but qmail itself cannot deliver them.
